...
import {
  ...
  Modal, Stack, TextInput, NumberInput, Select, SegmentedControl, Switch,
  Textarea, LoadingOverlay, ThemeIcon, Tooltip, Divider,
} from '@mantine/core';
import { useForm } from '@mantine/form';
import { useDisclosure } from '@mantine/hooks';
import { notifications } from '@mantine/notifications';
import {
  IconPlus, IconEdit, IconTrash, IconRepeat, IconArrowUpRight,
  IconArrowDownRight, IconCalendarDue, IconPlayerPause, IconPlayerPlay,
} from '@tabler/icons-react';
import api from '../lib/api';
import { useAuth } from '../contexts/AuthContext';

interface Fixo {
  id: string; nome: string; tipo: 'receita' | 'despesa'; valor: number;
  categoria: string; dia_vencimento: number | null; ativo: boolean; notas: string | null;
}

const categoriasReceita = ['Salário', 'Freelance fixo', 'Aluguel recebido', 'Pensão', 'Aposentadoria', 'Mesada', 'Outro'];
const categoriasDespesa = ['Moradia (Aluguel/Financiamento)', 'Condomínio', 'Internet', 'Celular', 'Energia elétrica', 'Água', 'Gás', 'Academia', 'Streaming (Netflix, Spotify...)', 'Plano de saúde', 'Seguro', 'Transporte (ônibus/metrô)', 'Escola/Faculdade', 'Assinatura', 'Outro'];
...
export default function Fixos() {
  ...
  const form = useForm({
    initialValues: { nome: '', tipo: 'despesa' as 'receita' | 'despesa', valor: '' as number | '', categoria: '', dia_vencimento: '' as number | '', ativo: true, notas: '' },
    validate: {
  ...
  const fetchFixos = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data } = await api.get('/api/fixos');
      setFixos(data || []);
    } catch (err: any) {
  ...
  useEffect(() => { if (user) fetchFixos(); }, [user, fetchFixos]);

  const handleOpenNew = () => { setEditingId(null); form.reset(); open(); };

  const handleOpenEdit = (f: Fixo) => {
    setEditingId(f.id);
    form.setValues({ nome: f.nome, tipo: f.tipo, valor: Number(f.valor), categoria: f.categoria, dia_vencimento: f.dia_vencimento || ('' as number | ''), ativo: f.ativo, notas: f.notas || '' });
    open();
  };

  const handleSubmit = async (values: typeof form.values) => {
    if (!user) return;
    const payload = { nome: values.nome, tipo: values.tipo, valor: Number(values.valor), categoria: values.categoria, dia_vencimento: values.dia_vencimento ? Number(values.dia_vencimento) : null, ativo: values.ativo, notas: values.notas || null };

    try {
      if (editingId) {
        await api.put(`/api/fixos/${editingId}`, payload);
        notifications.show({ title: 'Atualizado!', message: 'Alterações salvas.', color: 'teal' });
      } else {
        await api.post('/api/fixos', payload);
  ...
  const handleToggleAtivo = async (f: Fixo) => {
    try {
      await api.patch(`/api/fixos/${f.id}/toggle`);
      notifications.show({ title: f.ativo ? 'Pausado' : 'Reativado', message: `"${f.nome}" ${f.ativo ? 'pausado' : 'reativado'}.`, color: 'teal' });
      fetchFixos();
    } catch (err: any) {
      notifications.show({ title: 'Erro', message: err.response?.data?.error || err.message, color: 'red' });
    }
  ...
  return (
    <Box pos="relative" mih="60vh">
      ...
      <Group justify="space-between" mb="lg">
        <div><Title order={2} fw={700}>Fixos</Title><Text c="dimmed" size="sm">Rendas e despesas que se repetem todo mês</Text></div>
        <Button id="btn-novo-fixo" leftSection={<IconPlus size={16} />} onClick={handleOpenNew} style={{ background: 'linear-gradient(135deg, var(--mantine-color-teal-6), var(--mantine-color-teal-8))' }}>Novo fixo</Button>
      </Group>
                ...
                <Paper key={f.id} withBorder p="sm" radius="md" style={{ borderColor: 'var(--mantine-color-dark-5)', background: 'var(--mantine-color-dark-7)', opacity: f.ativo ? 1 : 0.5 }}>
                  <Group justify="space-between">
                    <div>
                      ...
                      <Group gap="xs"><Text size="xs" c="dimmed">{f.categoria}</Text>{f.dia_vencimento && <Badge color="teal" size="xs" variant="dot">Dia {f.dia_vencimento}</Badge>}</Group>
                    </div>
                    <Group gap={4}>
                      <Text fw={700} c="teal" size="sm">{formatCurrency(Number(f.valor))}</Text>
                      <Tooltip label={f.ativo ? 'Pausar' : 'Reativar'}><ActionIcon variant="subtle" color={f.ativo ? 'yellow' : 'teal'} size="sm" onClick={() => handleToggleAtivo(f)}>{f.ativo ? <IconPlayerPause size={14} /> : <IconPlayerPlay size={14} />}</ActionIcon></Tooltip>
                      <Tooltip label="Editar"><ActionIcon variant="subtle" color="blue" size="sm" onClick={() => handleOpenEdit(f)}><IconEdit size={14} /></ActionIcon></Tooltip>
                      <Tooltip label="Excluir"><ActionIcon variant="subtle" color="red" size="sm" onClick={() => handleDelete(f.id)}><IconTrash size={14} /></ActionIcon></Tooltip>
                    </Group>
                  </Group>
                </Paper>
              ))}
              ...
              {fixos.filter(f => f.tipo === 'despesa').map((f) => (
                <Paper key={f.id} withBorder p="sm" radius="md" style={{ borderColor: 'var(--mantine-color-dark-5)', background: 'var(--mantine-color-dark-7)', opacity: f.ativo ? 1 : 0.5 }}>
                  <Group justify="space-between">
                    <div>
                      <Group gap="xs"><Text size="sm" fw={600} td={f.ativo ? undefined : 'line-through'}>{f.nome}</Text>{!f.ativo && <Badge color="gray" size="xs" variant="light">Pausado</Badge>}</Group>
                      <Group gap="xs"><Text size="xs" c="dimmed">{f.categoria}</Text>{f.dia_vencimento && <Badge color="red" size="xs" variant="dot">Dia {f.dia_vencimento}</Badge>}</Group>
                    </div>
                    <Group gap={4}>
      ...
      <Modal opened={opened} onClose={close} title={<Text fw={700} size="lg">{editingId ? 'Editar fixo' : 'Novo fixo'}</Text>} size="md" radius="lg" centered overlayProps={{ blur: 3 }}>
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stack gap="md">
            <SegmentedControl fullWidth data={[{ label: '📉 Despesa fixa', value: 'despesa' }, { label: '📈 Receita fixa', value: 'receita' }]} color={form.values.tipo === 'receita' ? 'teal' : 'red'} {...form.getInputProps('tipo')} onChange={(v) => { form.setFieldValue('tipo', v as 'receita' | 'despesa'); form.setFieldValue('categoria', ''); }} />
            <TextInput label="Nome" placeholder="Ex: Salário, Internet Vivo, Academia..." size="md" {...form.getInputProps('nome')} />
            <NumberInput label="Valor mensal" placeholder="R$ 0,00" prefix="R$ " decimalSeparator="," thousandSeparator="." min={0.01} decimalScale={2} size="md" {...form.getInputProps('valor')} />
            <Select label="Categoria" placeholder="Selecione" data={currentCategorias} size="md" {...form.getInputProps('categoria')} />
            <NumberInput label="Dia do vencimento (opcional)" placeholder="Ex: 5, 10, 15..." min={1} max={31} size="md" leftSection={<IconCalendarDue size={16} />} {...form.getInputProps('dia_vencimento')} />
            {editingId && <Switch label="Ativo" description="Desative para pausar temporariamente" color="teal" {...form.getInputProps('ativo', { type: 'checkbox' })} />}
            <Textarea label="Notas (opcional)" placeholder="Observações..." autosize minRows={2} maxRows={3} {...form.getInputProps('notas')} />
            <Divider />
            <Group justify="flex-end"><Button variant="default" onClick={close}>Cancelar</Button><Button type="submit" style={{ background: form.values.tipo === 'receita' ? 'linear-gradient(135deg, var(--mantine-color-teal-6), var(--mantine-color-teal-8))' : 'linear-gradient(135deg, var(--mantine-color-red-6), var(--mantine-color-red-8))' }}>{editingId ? 'Salvar' : 'Adicionar'}</Button></Group>
          </Stack>
        </form>
      </Modal>
    </Box>
